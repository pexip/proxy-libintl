...
#ifdef _MSC_VER
#  define G_INTL_IMPORT __declspec(dllimport)
#else
#  define G_INTL_IMPORT extern
#endif
...
G_INTL_IMPORT char *g_libintl_gettext (const char *msgid) PROXY_LIBINTL_GNUC_FORMAT (1);

G_INTL_IMPORT char *g_libintl_dgettext (const char *domainname,
				 const char *msgid) PROXY_LIBINTL_GNUC_FORMAT (2);

G_INTL_IMPORT char *g_libintl_dcgettext (const char *domainname,
			const char *msgid,
			int         category) PROXY_LIBINTL_GNUC_FORMAT (2);

G_INTL_IMPORT char *g_libintl_ngettext (const char       *msgid1,
				 const char       *msgid2,
				 unsigned long int n) PROXY_LIBINTL_GNUC_FORMAT (1) PROXY_LIBINTL_GNUC_FORMAT (2);

G_INTL_IMPORT char *g_libintl_dngettext (const char       *domainname,
				  const char       *msgid1,
				  const char       *msgid2,
				  unsigned long int n) PROXY_LIBINTL_GNUC_FORMAT (2) PROXY_LIBINTL_GNUC_FORMAT (3);

G_INTL_IMPORT char *g_libintl_dcngettext (const char       *domainname,
				   const char       *msgid1,
				   const char       *msgid2,
				   unsigned long int n,
				   int               category) PROXY_LIBINTL_GNUC_FORMAT (2) PROXY_LIBINTL_GNUC_FORMAT (3);

G_INTL_IMPORT char *g_libintl_textdomain (const char *domainname);

G_INTL_IMPORT char *g_libintl_bindtextdomain (const char *domainname,
				       const char *dirname);

G_INTL_IMPORT char *g_libintl_bind_textdomain_codeset (const char *domainname,
						const char *codeset);
